...
long dijkstra(matrix_t &matrix) {
  ...
  // to find the elements in the left and right column
  for (std::size_t row = 0; row < rows; ++row) {
    edges.emplace_back(cols * rows, cols * row);
    weights.emplace_back(matrix[row][0]);
    edges.emplace_back(cols * (row + 1) - 1, cols * rows + 1);
  ...
  vertex_descriptor_t source = boost::vertex(rows * cols, graph);
